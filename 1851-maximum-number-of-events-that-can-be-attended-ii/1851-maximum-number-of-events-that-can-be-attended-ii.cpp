...
private:
    int maxValue(const vector<vector<int>>& events, int i, int k, vector<vector<int>>& mem) {
        ...
        return mem[i][k] = max(events[i][2] + maxValue(events, j, k - 1, mem), maxValue(events, i + 1, k, mem));
    }
};
